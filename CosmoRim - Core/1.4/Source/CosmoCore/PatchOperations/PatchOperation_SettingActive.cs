...
namespace CosmoCore
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = CosmoCoreMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!CosmoCoreMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (CosmoCoreMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
